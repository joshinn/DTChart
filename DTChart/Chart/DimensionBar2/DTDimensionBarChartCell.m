...
NSUInteger const LabelIndexTagPrefix = 9980;
...
@property(nonatomic) UIView *sectionLine;
/**
 * 记录触摸的subBBar对应的数据
 */
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(nullable NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _sectionLine = [UIView new];
        _sectionLine.backgroundColor = DTRGBColor(0x7b7b7b, 1);
        [self.contentView addSubview:_sectionLine];
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)sender {
    DTChartLabel *label = (DTChartLabel *) sender.view;
    if (label.text.length > 0) {
        id <DTDimensionBarChartCellDelegate> o = self.delegate;
        if ([o respondsToSelector:@selector(chartCellLongPress:data:gesture:)]) {
            [o chartCellLongPress:self data:self.mainData gesture:sender];
        }
    }
}
...
- (void)setCellData:(DTDimension2Model *)mainData second:(DTDimension2Model *)secondData prev:(DTDimension2Model *)prevData next:(DTDimension2Model *)nextData {
    ...
    if (self.labels.count != mainData.roots.count) {
        ...
        for (NSUInteger i = 0; i < mainData.roots.count; ++i) {
            ...
            label.tag = LabelIndexTagPrefix + i;
            label.userInteractionEnabled = self.enableSwipe;
        }
    } else {
        CGFloat x = 0;
        for (NSUInteger i = 0; i < mainData.roots.count; ++i) {
