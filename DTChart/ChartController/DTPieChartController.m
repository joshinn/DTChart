...
- (instancetype)initWithOrigin:(CGPoint)origin xAxis:(NSUInteger)xCount yAxis:(NSUInteger)yCount {
    if (self = [super initWithOrigin:origin xAxis:xCount yAxis:yCount]) {
        ...
        [_mainChart.toastView removeFromSuperview];
        [containerView addSubview:_mainChart.toastView];
        ...
        [_mainChart setPieChartTouchOneCompleteBlock:^NSString *(NSUInteger partIndex, NSString *partTitle, CGFloat partValue, CGFloat sum) {
            if (weakSelf.pieControllerTouchOneCompleteBlock) {
                return weakSelf.pieControllerTouchOneCompleteBlock(partIndex, partTitle, partValue, sum);
            }
            return nil;
        }];
    }
...
- (void)setHighlightTitle:(NSString *)highlightTitle {
    _mainChart.highlightTitle = highlightTitle;
    _secondChart.highlightTitle = highlightTitle;
}

- (void)setTouchMode:(DTPieChartTouchMode)touchMode {
    _mainChart.touchMode = touchMode;
    _secondChart.touchMode = touchMode;
}
